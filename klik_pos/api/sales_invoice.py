...
def _get_invoice_items_with_returns(invoice_id, customer):
	...
	items_query = """
		SELECT item_code, item_name, qty, rate, amount, description, item_group,
	...
	for item in items_data:
		...
		# Parse custom_ds_roofing_spec if it's a JSON string
		roofing_spec = item.get("custom_ds_roofing_spec")
		if roofing_spec:
			if isinstance(roofing_spec, str):
				try:
					roofing_spec = json.loads(roofing_spec)
...
def _add_roofing_spec_to_item(item_data, item):
	"""Add custom roofing spec if provided (for 'zn' category items)."""
	roofing_spec = item.get("custom_ds_roofing_spec")
	if roofing_spec:
		# Convert to JSON string if it's a list/dict
		if isinstance(roofing_spec, (list, dict)):
			item_data["custom_ds_roofing_spec"] = json.dumps(roofing_spec)
		else:
			item_data["custom_ds_roofing_spec"] = roofing_spec


def _add_description_to_item(item_data, item):
	"""Add custom description if provided (generated from roofing specs)."""
	description = item.get("custom_description")
	if description:
		item_data["custom_description"] = description
