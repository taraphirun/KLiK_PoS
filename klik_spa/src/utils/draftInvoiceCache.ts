...
export async function loadCachedItemsToCart(): Promise<boolean> {
  ...
  for (const item of cachedData.items) {
    // Build cartItem preserving all fields from cache, especially custom fields
    const cartItem = {
      id: item.id,
      name: item.name,
      ...
      // Preserve roofing spec data as-is, don't default to empty array
      // This prevents losing the spec data when items are restored from cache
      ...(item.custom_ds_roofing_spec && { custom_ds_roofing_spec: item.custom_ds_roofing_spec }),
    };

    console.log(`🔄 Loading cached item to cart:`, {
      itemId: item.id,
      itemName: item.name,
      quantity: item.quantity,
      custom_ds_roofing_spec: cartItem.custom_ds_roofing_spec,
    });
