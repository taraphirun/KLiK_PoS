...
interface CartState {
  ...
  updateItemCustomFields: (id: string, fields: { custom_ds_roofing_spec?: Array<{ straight: number; curve: number; end: number; quantity: number }>; custom_description?: string }) => void
...
export const useCartStore = create<CartState>()(
  persist(
    (set, get) => ({
      ...
      updateItemCustomFields: (id, fields) => {
        console.log(`🏪 Cart Store: Updating custom fields for item ${id}:`, fields);
        set((state) => ({
          cartItems: state.cartItems.map((item) =>
            item.id === id
              ? { ...item, ...fields }
