...
export const useCustomerActions = () => {
  ...
  const searchTelegramContact = async (
    searchQuery: string,
    searchType: 'phone' | 'username' | 'name' | 'all' = 'all'
  ): Promise<{ success: boolean; contacts: TelegramContact[]; message: string }> => {
    try {
      const response = await fetch('/api/method/klik_pos.api.customer.search_telegram_contact', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Frappe-CSRF-Token': csrfToken,
  ...
  const linkTelegramToCustomer = async (
    ...
    try {
      const response = await fetch('/api/method/klik_pos.api.customer.link_telegram_to_customer', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Frappe-CSRF-Token': csrfToken,
  ...
  const getCustomerTelegramLink = async (customerName: string): Promise<CustomerTelegramLink> => {
    try {
      const response = await fetch('/api/method/klik_pos.api.customer.get_customer_telegram_link', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Frappe-CSRF-Token': csrfToken,
